...
if torch.cuda.is_available():
    ...
    print("Mémoire allouée :", torch.cuda.memory_allocated(0)/1e9, "GB")
    print("Mémoire réservée :", torch.cuda.memory_reserved(0)/1e9, "GB")
...
class FourM(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.enc_mod_emb = nn.Embedding(num_embeddings=self.num_modalities, embedding_dim=dim)
        self.dec_mod_emb = nn.Embedding(num_embeddings=self.num_modalities, embedding_dim=dim)
                
        # Initialize Transformer encoder and decoder trunks
        self.encoder = TransformerTrunk(dim=dim, depth=enc_depth, head_dim=head_dim,
                                               mlp_ratio=mlp_ratio, use_bias=use_bias)
        self.decoder = TransformerDecoderTrunk(dim=dim, depth=dec_depth, head_dim=head_dim,
                                               mlp_ratio=mlp_ratio, use_bias=use_bias)
    ...
    def generate_one_modality_roar(
        ...
        start = 0
        for i in schedule:
            pos = perm[start:start + i].unsqueeze(0)
            dec_input_positions_list.append(pos)
            start += i
